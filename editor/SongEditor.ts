...
import { Instrument, Channel } from "../synth/song";
...
import { DrumsetRow, DrumsetView } from "./DrumsetRow"
...
export class SongEditor {
    ...
    private readonly _editMenu: HTMLSelectElement = select({ style: "width: 100%;" },
        ...
        option({ value: "customizePasteSelective" }, "Customize Selective Paste (" + ctrlSymbol + "Alt+V)"),
    ...
    private readonly _songEqFilterEditor: FilterEditor = new FilterEditor(this.doc, FilterEditorTypes.SongEq, false);
    ...
    private readonly _eqFilterEditor: FilterEditor = new FilterEditor(this.doc, FilterEditorTypes.EqFilter);
    ...
    private readonly _noteFilterEditor: FilterEditor = new FilterEditor(this.doc, FilterEditorTypes.NoteFilter);
    ...
    private readonly _unisonAntiPhasedBox = input({ type: "checkbox", style: "width: 1em; padding: 0; margin-left: 0.4em; margin-right: 4em;" });
    private readonly _unisonAntiPhasedRow: HTMLDivElement = div({ class: "selectRow dropFader" }, span({ class: "tip", style: "flex-shrink: 0;  font-size: smaller;", onclick: () => this._openPrompt("unisonAntiPhased") }, "‣ Antiphased: "), this._unisonAntiPhasedBox);
    private readonly _unisonBuzzesBox = input({ type: "checkbox", style: "width: 1em; padding: 0; margin-left: 0.4em; margin-right: 4em;" });
    private readonly _unisonBuzzesBoxRow: HTMLDivElement = div({ class: "selectRow dropFader" }, span({ class: "tip", style: "flex-shrink: 0;  font-size: smaller;", onclick: () => this._openPrompt("unisonBuzzes") }, "‣ Buzzes: "), this._unisonBuzzesBox);

    private readonly _unisonDropdownGroup: HTMLElement = div({ class: "editor-controls", style: "display: none; gap: 3px; margin-bottom: 0.5em;" }, this._unisonVoicesRow, this._unisonSpreadRow, this._unisonOffsetRow, this._unisonExpressionRow, this._unisonSignRow, this._unisonAntiPhasedRow, this._unisonBuzzesBoxRow);
    ...
    private readonly _monophonicNoteInputBox: HTMLInputElement = input({ style: "width: 2.35em; height: 1.5em; font-size: 80%; margin: 0.5em; vertical-align: middle;", id: "monophonicInputBox", type: "number", step: "1", min: 1, max: Config.maxChordSize, value: 1.0 });
    ...
    //SongEditor.ts
    readonly envelopeEditor: EnvelopeEditor = new EnvelopeEditor(this.doc, (id: number, submenu: number, subtype: string) => this._toggleDropdownMenu(id, submenu), (name: string, extraSettings: any) => this._openPrompt(name, extraSettings));
    ...
    */
    ...
    constructor() {
        ...
        this._unisonAntiPhasedBox.addEventListener("input", () => { this.doc.record(new ChangeUnisonAntiPhased(this.doc, this._unisonAntiPhasedBox.checked)) });
        this._unisonBuzzesBox.addEventListener("input", () => { this.doc.record(new ChangeUnisonBuzzing(this.doc, this._unisonBuzzesBox.checked)) });
    ...
    private _toggleDropdownMenu(dropdown: DropdownID, submenu: number = 0): void {
        ...
        switch (dropdown) {
            ...
            case DropdownID.EnvelopeSettings:
                ...
                break;
            case DropdownID.DrumsetEnvelopeSettings:
                target = this._drumsetRows[submenu].extraSettingsDropdown;
                this._drumsetRows[submenu].openExtraSettingsDropdown = this._drumsetRows[submenu].openExtraSettingsDropdown ? false : true;
                group = this._drumsetRows[submenu].extraSettingsDropdownGroup;
                shouldOpen = this._drumsetRows[submenu].openExtraSettingsDropdown;
                this._drumsetRows[submenu].render();
                break;
            default:
                throw new Error("No support for dropdown id " + dropdown);
        }
    ...
    private _setPrompt(promptName: string | null, extraSettings: any = {}): void {
        ...
        if (this.prompt) {
            if (this._wasPlaying && !(this.prompt instanceof Prompts.TipPrompt || this.prompt instanceof Prompts.LimiterPrompt || this.prompt instanceof Prompts.CustomScalePrompt || this.prompt instanceof Prompts.CustomChipPrompt || this.prompt instanceof Prompts.CustomFilterPrompt || this.prompt instanceof Prompts.VisualLoopControlsPrompt || this.prompt instanceof Prompts.SustainPrompt || this.prompt instanceof Prompts.HarmonicsEditorPrompt || this.prompt instanceof Prompts.SpectrumEditorPrompt)) {
        ...
        if (promptName) {
            switch (promptName) {
                case "export":
                    this.prompt = new Prompts.ExportPrompt(this.doc);
                    break;
                case "import":
                    this.prompt = new Prompts.ImportPrompt(this.doc);
                    break;
                case "songRecovery":
                    this.prompt = new Prompts.SongRecoveryPrompt(this.doc);
                    break;
                case "barCount":
                    this.prompt = new Prompts.SongDurationPrompt(this.doc);
                    break;
                case "beatsPerBar":
                    this.prompt = new Prompts.BeatsPerBarPrompt(this.doc);
                    break;
                case "moveNotesSideways":
                    this.prompt = new Prompts.MoveNotesSidewaysPrompt(this.doc);
                    break;
                case "channelSettings":
                    this.prompt = new Prompts.ChannelSettingsPrompt(this.doc);
                    break;
                case "limiterSettings":
                    this.prompt = new Prompts.LimiterPrompt(this.doc, this);
                    break;
                case "customScale":
                    this.prompt = new Prompts.CustomScalePrompt(this.doc);
                    break;
                case "customChipSettings":
                    this.prompt = new Prompts.CustomChipPrompt(this.doc, this);
                    break;
                case "customEQFilterSettings":
                    this.prompt = new Prompts.CustomFilterPrompt(this.doc, this, false);
                    break;
                case "customNoteFilterSettings":
                    this.prompt = new Prompts.CustomFilterPrompt(this.doc, this, true);
                    break;
                case "customSongEQFilterSettings":
                    this.prompt = new Prompts.CustomFilterPrompt(this.doc, this, false, true);
                    break;
                case "theme":
                    this.prompt = new Prompts.ThemePrompt(this.doc);
                    break;
                case "layout":
                    this.prompt = new Prompts.LayoutPrompt(this.doc);
                    break;
                case "recordingSetup":
                    this.prompt = new Prompts.RecordingSetupPrompt(this.doc);
                    break;
                case "multithreadingSetup":
                    this.prompt = new Prompts.MultithreadingSetupPrompt(this.doc);
                    break;
                case "exportInstrument":
                    this.prompt = new Prompts.InstrumentExportPrompt(this.doc);//, this);
                    break;
                case "importInstrument":
                    this.prompt = new Prompts.InstrumentImportPrompt(this.doc);//, this);
                    break;
                case "stringSustain":
                    this.prompt = new Prompts.SustainPrompt(this.doc);
                    break;
                case "addExternal":
                    this.prompt = new Prompts.AddSamplesPrompt(this.doc);
                    break;
                case "addPlugin":
                    this.prompt = new Prompts.PluginPrompt(this.doc);
                    break;
                case "generateEuclideanRhythm":
                    this.prompt = new Prompts.EuclideanRhythmPrompt(this.doc);
                    break;
                case "customTheme":
                    this.prompt = new Prompts.CustomThemePrompt(this.doc, this._patternEditor, this._trackArea, document.getElementById("beepboxEditorContainer")!);
                    break;
                case "visualLoopControls":
                    this.prompt = new Prompts.VisualLoopControlsPrompt(this.doc, this);
                    break;
                case "sampleLoadingStatus":
                    this.prompt = new Prompts.SampleLoadingStatusPrompt(this.doc);
                    break;
                case "configureShortener":
                    this.prompt = new Prompts.ShortenerConfigPrompt(this.doc);
                    break;
                ...
                case "selectiveRandom":
                    this.prompt = new Prompts.SelectiveRandomPastePrompt(this.doc, true);
                    break;
                case "selectivePaste":
                    this.prompt = new Prompts.SelectiveRandomPastePrompt(this.doc, false);
                    break;
                default:
                    this.prompt = new Prompts.TipPrompt(this.doc, promptName, extraSettings);
                    break;
            }

            if (this.prompt) {
                if (!(this.prompt instanceof Prompts.TipPrompt || this.prompt instanceof Prompts.LimiterPrompt || this.prompt instanceof Prompts.CustomChipPrompt || this.prompt instanceof Prompts.CustomFilterPrompt || this.prompt instanceof Prompts.VisualLoopControlsPrompt || this.prompt instanceof Prompts.SustainPrompt || this.prompt instanceof Prompts.HarmonicsEditorPrompt || this.prompt instanceof Prompts.SpectrumEditorPrompt || this.prompt instanceof Prompts.SequenceEditorPrompt )) {
    ...
    public whenUpdated = (): void => {
        ...
        if (!this.doc.song.getChannelIsMod(this.doc.channel)) {
            ...
            if (instrument.type != InstrumentType.mod) {
                ...
                if (instrument.type == InstrumentType.pickedString || instrument.unisonVoices == 1) {
                    //there's no good way to predict the zero crossings of a karplusser algorithm
                    //additionally, antiphasing doesn't mean anything on just one voice
                ...
                this._unisonBuzzesBoxRow.style.display = (instrument.unisonVoices == 1 ? "" : "none"); // hide unless voices == 1
            ...
            if (instrument.type == InstrumentType.customChipWave) {
                ...
                if (this.prompt instanceof Prompts.CustomChipPrompt) {
    ...
    private _whenKeyPressed = (event: KeyboardEvent): void => {
        ...
        if (this.prompt) {
            if (this.prompt instanceof Prompts.CustomChipPrompt || this.prompt instanceof Prompts.LimiterPrompt || this.prompt instanceof Prompts.CustomScalePrompt || this.prompt instanceof Prompts.CustomFilterPrompt) {
        ...
        if (
            ...
            || document.activeElement == this._lowerNoteLimitInputBox
