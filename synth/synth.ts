...
class EnvelopeComputer {
    ...
    public computeEnvelopes(instrument: Instrument, currentPart: number, tickTimeStart: number[], tickTimeStartReal: number, secondsPerTick: number, tone: Tone | null, timeScale: number[], instrumentState: InstrumentState, synth: Synth, channelIndex: number, instrumentIndex: number, perNote: boolean): void {
        ...
        for (let envelopeIndex: number = 0; envelopeIndex <= instrument.envelopeCount; envelopeIndex++) {
            ...
            if (envelopeIndex == instrument.envelopeCount) {
                ...
                if (synth.isModActive(Config.modulators.dictionary["individual envelope lower bound"].index, channelIndex, instrumentIndex) && envelopeSettings.tempEnvelopeLowerBound != null) { //modulation
                    perEnvelopeLowerBound = envelopeSettings.tempEnvelopeLowerBound!;
                }
                if (synth.isModActive(Config.modulators.dictionary["individual envelope upper bound"].index, channelIndex, instrumentIndex) && envelopeSettings.tempEnvelopeUpperBound != null) { //modulation
                    perEnvelopeUpperBound = envelopeSettings.tempEnvelopeUpperBound!;
                }
                ...
                if (envelope.type == EnvelopeType.sequence) sequence = synth.song?.sequences[waveform] || null;
            ...
            if (automationTarget.computeIndex != null && automationTarget.perNote == perNote) {
                ...
                let envelopeStart: number = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, globalEnvelopeSpeed, noteSecondsStartUnscaled, this.noteSecondsStart[envelopeIndex], beatTimeStart, timeSinceStart, noteSizeStart, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, steps, seed, waveform, defaultPitch, startPinTickAbsolute, sequence);
                if (prevSlideStart) {
                    const other: number = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, globalEnvelopeSpeed, prevNoteSecondsStartUnscaled, this.prevNoteSecondsStart[envelopeIndex], beatTimeStart, timeSinceStart, prevNoteSize, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, steps, seed, waveform, defaultPitch, startPinTickAbsolute, sequence);
                    envelopeStart += (other - envelopeStart) * prevSlideRatioStart;
                }
                if (nextSlideStart) {
                    const other: number = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, globalEnvelopeSpeed, 0.0, 0.0, beatTimeStart, timeSinceStart, nextNoteSize, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, steps, seed, waveform, defaultPitch, startPinTickAbsolute, sequence);
                    envelopeStart += (other - envelopeStart) * nextSlideRatioStart;
                }
                let envelopeEnd: number = envelopeStart;
                if (isDiscrete == false) {
                    envelopeEnd = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, globalEnvelopeSpeed, noteSecondsEndUnscaled, this.noteSecondsEnd[envelopeIndex], beatTimeEnd, timeSinceStart, noteSizeEnd, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, steps, seed, waveform, defaultPitch, startPinTickAbsolute, sequence);
                    if (prevSlideEnd) {
                        const other: number = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, globalEnvelopeSpeed, prevNoteSecondsEndUnscaled, this.prevNoteSecondsEnd[envelopeIndex], beatTimeEnd, timeSinceStart, prevNoteSize, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, steps, seed, waveform, defaultPitch, startPinTickAbsolute, sequence);
                        envelopeEnd += (other - envelopeEnd) * prevSlideRatioEnd;
                    }
                    if (nextSlideEnd) {
                        const other: number = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, globalEnvelopeSpeed, 0.0, 0.0, beatTimeEnd, timeSinceStart, nextNoteSize, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, steps, seed, waveform, defaultPitch, startPinTickAbsolute, sequence);
    ...
    public static computeEnvelope(envelope: Envelope, perEnvelopeSpeed: number, globalEnvelopeSpeed: number, unspedTime: number, time: number, beats: number, timeSinceStart: number, noteSize: number, pitch: number, inverse: boolean, perEnvelopeLowerBound: number, perEnvelopeUpperBound: number, steps: number, seed: number, waveform: number, defaultPitch: number, notePinStart: number, sequence: SequenceSettings | null): number {
        ...
        switch (envelope.type) {
            ...
            case EnvelopeType.twang:
                if (inverse) {
                    return perEnvelopeUpperBound - boundAdjust * (1.0 / (1.0 + time));
                } else {
                    return boundAdjust / (1.0 + time) + perEnvelopeLowerBound;
                }
            case EnvelopeType.swell:
                if (inverse) {
                    return boundAdjust / (1.0 + time) + perEnvelopeLowerBound; //swell is twang's inverse... I wonder if it would be worth it to just merge the two :/
                } else {
                    return perEnvelopeUpperBound - boundAdjust / (1.0 + time);
                }
            case EnvelopeType.lfo:
                switch (waveform) {
                    case LFOEnvelopeTypes.sine:
                        if (inverse) {
                            return (perEnvelopeUpperBound / 2) + boundAdjust * Math.cos(beats * 2.0 * Math.PI) * 0.5 + (perEnvelopeLowerBound / 2);
                        } else {
                            return (perEnvelopeUpperBound / 2) - boundAdjust * Math.cos(beats * 2.0 * Math.PI) * 0.5 + (perEnvelopeLowerBound / 2);
                        }
                    case LFOEnvelopeTypes.square:
                        if (inverse) {
                            return (Math.cos(beats * 2.0 * Math.PI + 3 * Math.PI / 2) < 0) ? perEnvelopeUpperBound : perEnvelopeLowerBound;
                        } else {
                            return (Math.cos(beats * 2.0 * Math.PI + 3 * Math.PI / 2) < 0) ? perEnvelopeLowerBound : perEnvelopeUpperBound;
                        }
                    case LFOEnvelopeTypes.triangle:
                        if (inverse) {
                            return (perEnvelopeUpperBound / 2) - (boundAdjust / Math.PI) * Math.asin(Math.sin((Math.PI / 2) + beats * Math.PI * 2.0)) + (perEnvelopeLowerBound / 2);
                        } else {
                            return (perEnvelopeUpperBound / 2) + (boundAdjust / Math.PI) * Math.asin(Math.sin((Math.PI / 2) + beats * Math.PI * 2.0)) + (perEnvelopeLowerBound / 2);
                        }
                    case LFOEnvelopeTypes.sawtooth:
                        if (inverse) {
                            return perEnvelopeUpperBound - (beats) % 1 * boundAdjust;
                        } else {
                            return (beats) % 1 * boundAdjust + perEnvelopeLowerBound;
                        }
                    case LFOEnvelopeTypes.trapezoid:
                        let trap: number = 0;
                        if (inverse) {
                            trap = (perEnvelopeUpperBound / 2) - (boundAdjust * 2 / Math.PI) * Math.asin(Math.sin((Math.PI / 2) + beats * Math.PI * 2.0)) + (perEnvelopeLowerBound / 2);
                        } else {
                            trap = (perEnvelopeUpperBound / 2) + (boundAdjust * 2 / Math.PI) * Math.asin(Math.sin((Math.PI / 2) + beats * Math.PI * 2.0)) + (perEnvelopeLowerBound / 2);
                        }
                    ...
                    case LFOEnvelopeTypes.steppedTri:
                        if (steps <= 1) return 1;
                        let tri: number = 0.5 + (inverse ? -1 : 1) * (1 / Math.PI) * Math.asin(Math.sin((Math.PI / 2) + beats * Math.PI * 2.0));
            ...
            case EnvelopeType.tremolo2: //kept only for drumsets right now
                if (inverse) {
                    return (perEnvelopeUpperBound / 4) + boundAdjust * Math.cos(beats * 2.0 * Math.PI) * 0.25 + (perEnvelopeLowerBound / 4); //inverse works strangely with tremolo2. If I ever update this I'll need to turn all current versions into tremolo with bounds
                } else {
                    return 0.5 + (perEnvelopeUpperBound / 4) - boundAdjust * Math.cos(beats * 2.0 * Math.PI) * 0.25 - (perEnvelopeLowerBound / 4);
                }
            ...
            case EnvelopeType.flare:
                const attack: number = 0.25 / Math.sqrt(globalEnvelopeSpeed * perEnvelopeSpeed); //flare and blip need to be handled a little differently with envelope speeds. I have to use the old system
                if (inverse) {
                    return perEnvelopeUpperBound - boundAdjust * (unspedTime < attack ? unspedTime / attack : 1.0 / (1.0 + (unspedTime - attack) * globalEnvelopeSpeed * perEnvelopeSpeed));
                } else {
                    return boundAdjust * (unspedTime < attack ? unspedTime / attack : 1.0 / (1.0 + (unspedTime - attack) * globalEnvelopeSpeed * perEnvelopeSpeed)) + perEnvelopeLowerBound;
                }
            case EnvelopeType.decay:
                if (inverse) {
                    return perEnvelopeUpperBound - boundAdjust * Math.pow(2, -time);
                } else {
                    return boundAdjust * Math.pow(2, -time) + perEnvelopeLowerBound;
                }
            case EnvelopeType.blip:
                if (inverse) {
                    return perEnvelopeUpperBound - boundAdjust * +(unspedTime < (0.25 / Math.sqrt(globalEnvelopeSpeed * perEnvelopeSpeed)));
                } else {
                    return boundAdjust * +(unspedTime < (0.25 / Math.sqrt(globalEnvelopeSpeed * perEnvelopeSpeed))) + perEnvelopeLowerBound;
                }
            case EnvelopeType.wibble:
                let temp = 0.5 - Math.cos(beats) * 0.5;
                temp = 1.0 / (1.0 + time * (temp / (1.5)));
            ...
            case EnvelopeType.fall: {
                if (inverse) {
                    return Math.min(Math.max(perEnvelopeLowerBound, perEnvelopeUpperBound - boundAdjust * Math.sqrt(Math.max(1.0 - time / 2, 0))), perEnvelopeUpperBound);
                } else {
                    return Math.max(perEnvelopeLowerBound, boundAdjust * Math.sqrt(Math.max(1.0 - time / 2, 0)) + perEnvelopeLowerBound);
                }
            }
            case EnvelopeType.sequence: {
                if (sequence == null) return 0;
                const t: number = sequence.looped ? beats : time;
                const beat: number = Math.floor(t);
                const frac: number = t - beat;
    ...
    public computePitchEnvelope(startNote: number, endNote: number, inverse: boolean, envelopeLowerBound: number, envelopeUpperBound: number, isNoise: boolean, pitch: number = 0): number {
        if (startNote > endNote) { //Reset if values are improper
            startNote = 0;
            endNote = isNoise ? Config.drumCount - 1 : Config.maxPitch;
        }
...
class InstrumentState {
    ...
    public pluginR: BeepBoxEffectPlugin | null = null; //if we're using a stereo sample but a mono plugin, create a separate one for the other channel
    ...
    public resetAllEffects(): void {
        ...
        if (this.pluginR) this.pluginR.reset();
...
export class Synth extends SynthTemplate {
    ...
    private static readonly chipFunctionCache: Function[] = Array(Config.unisonVoicesMax + 1).fill(undefined);
    private static readonly harmonicsFunctionCache: Function[] = Array(Config.unisonVoicesMax + 1).fill(undefined);
    private static readonly pickedStringFunctionCache: Function[] = Array(Config.unisonVoicesMax + 1).fill(undefined); // keep in sync with the number of unison voices.
    private static readonly pulseFunctionCache: Function[] = Array(Config.unisonVoicesMax + 1).fill(undefined);
    private static readonly supersawFunctionCache: Function[] = Array(Config.unisonVoicesMax + 1).fill(undefined);
    private static readonly noiseFunctionCache: Function[] = Array(Config.unisonVoicesMax + 1).fill(undefined);
    private static readonly spectrumFunctionCache: Function[] = Array(Config.unisonVoicesMax + 1).fill(undefined);
    private static readonly drumFunctionCache: Function[] = Array(Config.unisonVoicesMax + 1).fill(undefined);
    public static readonly effectsFunctionCache: Function[] = Array(1 << (EffectType.length - 5)).fill(undefined); // keep in sync with the number of post-process effects.
    ...
    */
    ...
    private computeTone(song: Song, channelIndex: number, samplesPerTick: number, tone: Tone, released: boolean, shouldFadeOutFast: boolean): void {
        ...
        if (effectsIncludePitchShift(instrument.effects)) {
            let usePitchShiftStart: number = instrument.pitchShift;
            let usePitchShiftEnd: number = instrument.pitchShift;
        ...
        if (instrument.type == InstrumentType.drumset) {
            ...
            const drumsetFilterEnvelopeEnd = drumsetEnvelopeComputer.drumsetFilterEnvelopeEnd * (1.0 + drumsetEnvelopeComputer.drumsetFilterEnvelopeEnd);
        ...
        if (instrument.type == InstrumentType.fm || instrument.type == InstrumentType.fm6op) {
            ...
            if (instrument.type == InstrumentType.chip || instrument.type == InstrumentType.customChipWave || instrument.type == InstrumentType.harmonics || instrument.type == InstrumentType.pickedString || instrument.type == InstrumentType.spectrum || instrument.type == InstrumentType.pwm || instrument.type == InstrumentType.noise || instrument.type == InstrumentType.drumset) {
                ...
                if ((!instrumentState.unisonInitialized || !tone.unisonHasUpdated) && instrument.unisonAntiPhased && (instrument.type == InstrumentType.pwm || instrument.type == InstrumentType.drumset || instrumentState.waveL) && instrument.type != InstrumentType.pickedString) {
                    if (instrument.type == InstrumentType.spectrum) {
                        for (let i: number = 0; i < Config.unisonVoicesMax; i++) {
                        ...
                        function wavePoint(phase: number): number {
                            ...
                            const flooredPhase: number = (phase | 0) % instrumentState.waveL!.length;
                            const remainder: number = phase - flooredPhase;
                            const flooredPhase2: number = flooredPhase + 1 >= instrumentState.waveL!.length ? flooredPhase + 1 - instrumentState.waveL!.length : flooredPhase + 1;
                        ...
                        for (let i = 1; i <= steps; i++) {
                            let phaseNew = i / steps * (instrument.type == InstrumentType.pwm ? 1 : instrumentState.waveL!.length);
    ...
    public static getInstrumentSynthFunction(instrument: Instrument): Function {
        if (instrument.type == InstrumentType.fm) {
            ...
            if (instrument.isUsingAdvancedLoopControls) {
                return Synth.loopableChipSynth(instrument);
            }
            return Synth.chipSynth(instrument);
        } else if (instrument.type == InstrumentType.customChipWave) {
            return Synth.chipSynth(instrument);
        } else if (instrument.type == InstrumentType.harmonics) {
            return Synth.harmonicsSynth(instrument);
        } else if (instrument.type == InstrumentType.pwm) {
            return Synth.pulseWidthSynth(instrument);
        } else if (instrument.type == InstrumentType.supersaw) {
            return Synth.supersawSynth(instrument);
        } else if (instrument.type == InstrumentType.pickedString) {
            return Synth.pickedStringSynth(instrument);
        } else if (instrument.type == InstrumentType.noise) {
            return Synth.noiseSynth(instrument);
        } else if (instrument.type == InstrumentType.spectrum) {
            return Synth.spectrumSynth(instrument);
        } else if (instrument.type == InstrumentType.drumset) {
            return Synth.drumsetSynth(instrument);
        } else if (instrument.type == InstrumentType.mod) {
    ...
    static loopableChipSynth(instrument: Instrument): Function {
        // @TODO:
        ...
        //   user.
        const voiceCount: number = Math.max(2, instrument.unisonVoices);
        let chipFunction: Function = Synth.loopableChipFunctionCache[instrument.unisonVoices];
        if (chipFunction == undefined) {
            //TODO Pull out the chipwave loop mode and cache based on that as well
        ...
        return (synth: Synth, bufferIndex: number, roundedSamplesPerTick: number, tone: Tone, instrumentState: InstrumentState) => chipFunction(synth, bufferIndex, roundedSamplesPerTick, tone, instrumentState);
    }

    private static chipSynth(instrument: Instrument): Function {
        const voiceCount: number = Math.max(2, instrument.unisonVoices);
        let chipFunction: Function = Synth.chipFunctionCache[instrument.unisonVoices];
        if (chipFunction == undefined) {
            let chipSource: string = `return (synth, bufferIndex, roundedSamplesPerTick, tone, instrumentState) => {
        const aliases = (effectsIncludeDistortion(instrumentState.effects) && instrumentState.aliases);
        const stereoChannels = instrumentState.stereoChannels;
        ...
        for (let sampleIndex = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
            ...
            if (aliases) {
                ...
                `
            const sampleListAliasedL: string[] = [];
            for (let voice: number = 0; voice < voiceCount; voice++) {
                sampleListAliasedL.push("inputSampleL" + voice + (voice != 0 ? " * unisonSign" : ""));
            }

            const sampleListAliasedR: string[] = [];
            for (let voice: number = 0; voice < voiceCount; voice++) {
            ...
            chipSource += `
                ...
                prevWaveIntegralR# = nextWaveIntegralR#;
                        `
            const sampleListUnaliasedL: string[] = [];
            for (let voice: number = 0; voice < voiceCount; voice++) {
                sampleListUnaliasedL.push("inputSampleL" + voice + (voice != 0 ? " * unisonSign" : ""));
            }

            const sampleListUnaliasedR: string[] = [];
            for (let voice: number = 0; voice < voiceCount; voice++) {
    ...
    private static harmonicsSynth(instrument: Instrument): Function {
        const voiceCount: number = Math.max(2, instrument.unisonVoices);
        let harmonicsFunction: Function = Synth.harmonicsFunctionCache[instrument.unisonVoices];
        ...
        tone.initialNoteFilterInputL2 = initialFilterInput2;
            ...
            Synth.harmonicsFunctionCache[instrument.unisonVoices] = harmonicsFunction;
        }
        return (synth: Synth, bufferIndex: number, roundedSamplesPerTick: number, tone: Tone, instrumentState: InstrumentState) => harmonicsFunction(synth, bufferIndex, roundedSamplesPerTick, tone, instrumentState);
    }

    private static pickedStringSynth(instrument: Instrument): Function {
        ...
        return (synth: Synth, bufferIndex: number, roundedSamplesPerTick: number, tone: Tone, instrumentState: InstrumentState) => pickedStringFunction(synth, bufferIndex, roundedSamplesPerTick, tone, instrumentState);
    }
    ...
    private static effectsSynth(synth: Synth, outputDataL: Float32Array, outputDataR: Float32Array, bufferIndex: number, runLength: number, instrumentState: InstrumentState): void {
        ...
        const stereoChannels: number = (instrumentState.type == InstrumentType.chip) ? instrumentState.stereoChannels : 0; //TODO: make an instrumentIsStereo function
        ...
        if (effectsFunction == undefined) {
            ...
            //determine whether or not we need to use the stereo or mono version of each effect
            let monoGranular: boolean = stereoChannels < 2;
            let monoDistortion: boolean = stereoChannels < 2;
            let monoBitcrush: boolean = stereoChannels < 2;
            let monoRingMod: boolean = stereoChannels < 2;
            let monoEqFilter: boolean = stereoChannels < 2;
        ...
        ${ !monoGranular ? "granularOutputR += grainSampleR * grainEnvelope;" : ""}
            ...
            if (usesPlugin && instrumentState.plugin) {
                ...
                if (typeof instrumentState.plugin.effectOrderIndex == "number") {
                    ...
                    //TODO: Allow rearranging after the function has been made
    ...
    private static pulseWidthSynth(instrument: Instrument): Function {
        const voiceCount: number = Math.max(2, instrument.unisonVoices);
        let pulseFunction: Function = Synth.pulseFunctionCache[instrument.unisonVoices];
        ...
        tone.initialNoteFilterInputL2 = initialFilterInput2;
            ...
            Synth.pulseFunctionCache[instrument.unisonVoices] = pulseFunction;
        }

        return (synth: Synth, bufferIndex: number, roundedSamplesPerTick: number, tone: Tone, instrumentState: InstrumentState) => pulseFunction(synth, bufferIndex, roundedSamplesPerTick, tone, instrumentState);
    }
    ...
    private static noiseSynth(instrument: Instrument): Function {
        const voiceCount: number = Math.max(2, instrument.unisonVoices);
        let noiseFunction: Function = Synth.noiseFunctionCache[instrument.unisonVoices];
        ...
        tone.initialNoteFilterInputL2 = initialFilterInput2;
            ...
            Synth.noiseFunctionCache[instrument.unisonVoices] = noiseFunction;
        }
        return (synth: Synth, bufferIndex: number, runLength: number, tone: Tone, instrumentState: InstrumentState) => noiseFunction(synth, bufferIndex, runLength, tone, instrumentState);
    }


    private static spectrumSynth(instrument: Instrument): Function {
        const voiceCount: number = Math.max(2, instrument.unisonVoices);
        let spectrumFunction: Function = Synth.spectrumFunctionCache[instrument.unisonVoices];
        ...
        tone.initialNoteFilterInputL2 = initialFilterInput2;
            ...
            Synth.spectrumFunctionCache[instrument.unisonVoices] = spectrumFunction;
        }
        return (synth: Synth, bufferIndex: number, runLength: number, tone: Tone, instrumentState: InstrumentState) => spectrumFunction(synth, bufferIndex, runLength, tone, instrumentState);
    }

    private static drumsetSynth(instrument: Instrument): Function {
        const voiceCount: number = Math.max(2, instrument.unisonVoices);
        let drumFunction: Function = Synth.drumFunctionCache[instrument.unisonVoices];
        ...
        tone.initialNoteFilterInputL2 = initialFilterInput2;
            ...
            Synth.drumFunctionCache[instrument.unisonVoices] = drumFunction;
        }
        return (synth: Synth, bufferIndex: number, runLength: number, tone: Tone, instrumentState: InstrumentState) => drumFunction(synth, bufferIndex, runLength, tone, instrumentState);
    }
