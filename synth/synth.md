...
The main thread intermediary between the editor and the synth. Handles the actual sending of messages to the synth thread and the mod channel information needed for the editor to display. 

# [processor.ts](./processor.ts)
The entry point for the AudioWorkletNode. It reads audio samples from a ring buffer and outputs them, as well as calculating lag.
...
# [song.ts](./song.ts)
Contains the declarations for different data structures like Songs or Instruments, a well as the abstract synth class that the main and synth threads inherit from. 
...
# [synthThread.ts](./synthThread.ts)
The entry point for the actual synth worker. It handles the receiving of messages from the main thread, and sending any messages if needed. 