...
export class SynthProcessor extends AudioWorkletProcessor {
    ...
    public synthesize(outputDataL: Float32Array, outputDataR: Float32Array, outputDataLLength: number, playSong: boolean = true): void {
        ...
        if (this.outputDataLUnfiltered == null || this.outputDataLUnfiltered.length < outputDataLLength) {
            this.outputDataLUnfiltered = new Float32Array(outputDataLLength);
            this.outputDataRUnfiltered = new Float32Array(outputDataLLength);
        } else {
        ...
        if (this.tempMonoInstrumentSampleBuffer == null || this.tempMonoInstrumentSampleBuffer.length < outputDataLLength) {
            this.tempMonoInstrumentSampleBuffer = new Float32Array(outputDataLLength);
        }
        ...
        while (bufferIndex < outputDataLLength && !ended) {

            this.nextBar = this.getNextBar();
            if (this.nextBar >= song.barCount) this.nextBar = null;

            const samplesLeftInBuffer: number = outputDataLLength - bufferIndex;
    ...
    private static supersawSynth(synth: SynthProcessor, bufferIndex: number, runLength: number, tone: Tone, instrumentState: InstrumentState): void {
        ...
        let supersawFunction: Function = SynthProcessor.supersawFunctionCache[0]; //currently only one supersaw function can exist in a given song / mod. Change to an array if you desire to support multiple by, for example, having unisons on supersaws
        ...
        tone.initialNoteFilterInput2 = initialFilterInput2;
            ...
            SynthProcessor.supersawFunctionCache[0] = supersawFunction;
        }
